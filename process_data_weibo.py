...
def write_data(flag, image, text_only):
    ...
    def read_post(flag):
        ...
        #{'3874351616957777': 28, '3924019927281845': 6, '3803929878271778': 21,
        post_content = []
        labels = []
        image_ids = []
        twitter_ids = []
...
def add_unknown_words(word_vecs, vocab, min_df=1, k=32):
    ...
    """
    for word in vocab:
        print(isinstance(word.encode('utf-8'), bytes))

        if word.encode('utf-8') not in word_vecs and vocab[word] >= min_df:
            print("hi")
            print(word_vecs.shape)
            word_vecs[bytes((create_jsonlines(word_vecs)), encoding='utf8')] = np.random.uniform(-0.25, 0.25, k)
'''

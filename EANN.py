...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels, event_labels) in enumerate(train_loader):
            ...
            ## Fake or Real loss
            class_loss = criterion(class_outputs, train_labels.to(torch.long))
            # Event Loss
            domain_loss = criterion(domain_outputs, event_labels.to(torch.long))
        ...
        for i, (validate_data, validate_labels, event_labels) in enumerate(validate_loader):
            ...
            vali_loss = criterion(validate_outputs, validate_labels.to(torch.long))
